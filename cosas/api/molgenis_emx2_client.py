...
class Molgenis:
  ...
  def signin(self, email, password):
    ...
    response = self.session.post(
      url=self._url_signin, json={'query': query, 'variables': variables}
    )
    ...
    if status.get('status') == 'SUCCESS':
      ...
      self.cli.alert_error(f'unable to sign into {msg}: {status.get("message")}')
  ...
  def importData(self, table: str = None, data: str = None):
    ...
    @param table name of the table
    @param data a string containing your data as a text/csv object
